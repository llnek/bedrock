 ...
 *
 */
class MultiStepFlow(job:Job) extends Workflow(job) {
  ...
  def onStart() : Activity = {
    ...
    new Block(AuthUser).chain(GetProfile).chain(Provision)
            .chain(FinalTest)
  }

}

class MultiStepFlowPreamble(j:Job) extends Workflow(j) {

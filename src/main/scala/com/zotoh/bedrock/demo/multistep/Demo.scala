...
class Demo(job:Job) extends Workflow(job) {
  ...
  private val AuthUser = new Switch()
          .withChoice("facebook", Auth.getAuthMtd("facebook"))
          .withChoice("google+", Auth.getAuthMtd("google+"))
          .withChoice("openid", Auth.getAuthMtd("openid"))
          .withDef(Auth.getAuthMtd("db"))
  ...
  private val prov_ami = new While().withBody(new PTask(perm_ami)).withExpr(
      ...
      new BoolExpr() {
        def eval(j:Job) = {
          var c=0
          j.slot("ami_count") match {
            case Some(n:Int) => c= n+1
            case _ =>
          }
          j.setSlot("ami_count", c)
  ...
  def onStart() = {
    ...
    new Block(AuthUser).chain(GetProfile).chain(Provision).chain(FinalTest)
  }

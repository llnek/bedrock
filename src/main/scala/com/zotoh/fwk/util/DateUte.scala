 ...
 *
 */
object DateUte extends Consts with CoreImplicits {
  ...
  def parseTimestamp(t:String):Option[JTStamp] =
    try { Some(JTStamp.valueOf(t)) } catch { case _ => None }
  ...
  def parseDate(t:String, fmt:String):Option[JDate] = {
    if (isEmpty(t) || isEmpty(fmt)) None else {
  ...
  def fmtDate(dt:JDate, pattern:String, tz:Option[JTZone]):String = {
    if (dt==null || isEmpty(pattern)) "" else {

...
abstract class CmdLineQ( private var _id:String, private var _question:String,
   ...
   * @param b
   */
  def setMust(b:Boolean) = { _mandatory=b ; this }
   ...
   * @param a
   */
  def setDftAnswer(a:String) = { _dftAnswer=nsb(a) ; this }
   ...
   * @param c
   */
  def setChoices(c:String) = { _choices=nsb(c) ; this }

  /**
   * @param props
   */
  def setOutput(props:JPS) = {    _props=props ; this  }
  ...
  def setAnswer(a:String) = {
    _answer= nsb( if (isEmpty(a)) _dftAnswer else a)

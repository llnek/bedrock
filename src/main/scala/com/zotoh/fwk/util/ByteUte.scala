...
import java.io.{ByteArrayOutputStream=>BAOS,DataInputStream,DataOutputStream}
...
object ByteUte {
  ...
  def readAsLong(bits:Array[Byte]) = {
    new DataInputStream( asStream(bits)).readLong()
  }
  ...
  def readAsInt(bits:Array[Byte]) = {
    new DataInputStream( asStream(bits)).readInt()
  }

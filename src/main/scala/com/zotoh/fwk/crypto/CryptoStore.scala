...
abstract class CryptoStore {
  ...
  private def ilog() { _log = getLogger(classOf[CryptoStore]) }
  ...
  def addKeyEntity(bits:InputStream, pwd:String) {
    ...
    val key= tmp.getEntry( tmp.aliases().nextElement(),
        pp ).asInstanceOf[PrivateKeyEntry ]
   ...
   * @throws IOException
   */
  def init(f:File, pwd:String):CryptoStore = {
   ...
   * @throws IOException
   */
  def init( inp:InputStream, pwd:String):CryptoStore = {
    init(pwd)
   ...
   * @throws IOException
   */
  def init( pwd:String ):CryptoStore = {

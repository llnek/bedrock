...
import java.io.{ByteArrayOutputStream=>BAOS,ByteArrayInputStream=>BAIS,File,InputStreamReader}
...
object Crypto {
  ...
  @transient private var _log= getLogger(classOf[Crypto])
  def tlog():Logger = _log
  ...
  def mkCSR( keyLength:Int, dnStr:String, fmt:CertFormat) = {
    ...
    tlog().debug("Crypto: mkCSR: dnStr= {}, key-len= {}", dnStr, asJObj(keyLength))
  ...
  def mkPKCS12(friendlyName:String, keyPEM:Array[Byte], certPEM:Array[Byte],
    ...
    val ct= CryptoUte.bitsToCert(certPEM) match {
      case Some(c) => c.getTrustedCertificate()
      case _ => null
    }
    val ss= PKCSStore().createKeyStore()
    val rdr=new InputStreamReader( new BAIS(keyPEM))
    ...
    IOUte.writeFile(out, baos.toByteArray())
  }
  ...
  def mkSSV1PKCS12(friendlyName:String, start:JDate, end:JDate,
    ...
    ks.load(null, null)
    val keyPair= mkKeyPair("RSA", keyLength)
    mk_SSV1(ks, keyPair, DEF_ALGO, friendlyName, start,end,dnStr,pwd,keyLength, out)
  }
  ...
  def mkSSV1JKS(friendlyName:String, start:JDate, end:JDate, dnStr:String, pwd:String,
    ...
    ks.load(null, null)
    val keyPair= mkKeyPair("DSA", keyLength)
    mk_SSV1(ks, keyPair, "SHA1withDSA", friendlyName, start,end,
      dnStr,pwd,keyLength, out)
  }
  ...
  private def mk_SSV1(ks:KeyStore, kp:KeyPair, algo:String, friendlyName:String,
    ...
    tlog().debug("Crypto:createSSV1: dn={}, key-len={}",  dnStr, asJObj(keyLength))
...
//    "SHA1withRSA"
   ...
   * @throws NoSuchAlgorithmException
   */
  def newDigestInstance(algo:SigningAlgo) = CryptoUte.newMsgDigest(algo.toString())
  ...
  private def fmtPEM(top:String, end:String, bits:Array[Byte]) = {
    val baos= new BAOS()
    baos.write( CoreUte.asBytes(top))
    val bs=Base64.encode(bits)
    val bb= Array[Byte](1)
    var pos=0
    for ( i <- 0 until bs.length) {
      if (pos > 0 && (pos % 64) == 0) {
        baos.write( CoreUte.asBytes("\n"))
      }
      pos += 1
      bb(0)=bs(i)
      baos.write(bb)
    }
    baos.write( CoreUte.asBytes(end))
    baos.toByteArray()
  }
    ...
    try {
      ...
      ks.getEntry( nm, new PasswordProtection("Password1".toCharArray())) match {
        case k:PrivateKeyEntry =>
          ...
          ks.load(null, null)
          ks.setKeyEntry(nm, k.getPrivateKey(),
            "Password1".toCharArray(), k.getCertificateChain())

...
import com.zotoh.fwk.mime.MimeUte._
 ...
 *
 */
object CryptoUte extends CoreImplicits {

  private def ilog() { _log = getLogger(classOf[CryptoUte])  }
   ...
   * @return
   */
  def newMimeMsg(user:String, pwd:String) = {
    new MimeMessage(newSession(user, pwd))
  }
  ...
  def newSession(user:String, pwd:String) = {
    Session.getInstance(getProperties(),
        if (isEmpty(user)) null else new DefaultAuthenticator(user, pwd) )
  }
  ...
  def isSigned(obj:Any):Boolean = {
    val inp= MimeUte.maybeAsStream(obj)
    if (inp==null) {
      obj match {
        case mp:Multipart => return MimeUte.isSigned( mp.getContentType())
        case _ => throw new Exception("Invalid content: " + safeGetClzname(obj))
      }
    }
    // else
    try {
  ...
  def isCompressed(obj:Any):Boolean = {
    ...
    if (inp==null) {
      obj match {
        case mp:Multipart => return MimeUte.isCompressed(mp.getContentType())
        case bp:BodyPart => return MimeUte.isCompressed(bp.getContentType())
        case _ => throw new Exception("Invalid content: " + safeGetClzname(obj))
      }
    }
    // else
    try {
      MimeUte.isCompressed(newMimeMsg(inp).getContentType())
    }
    finally {
      safeReset(inp)
    }
  ...
  def isEncrypted(obj:Any):Boolean = {
    ...
    if (inp==null) {
      obj match {
        case mp:Multipart => return MimeUte.isEncrypted( mp.getContentType())
        case bp:BodyPart => return MimeUte.isEncrypted( bp.getContentType())
        case _ => throw new Exception("Invalid content: " + safeGetClzname(obj))
      }
    }
    // else
    try {
      MimeUte.isEncrypted(newMimeMsg(inp).getContentType())
    }
    finally {
      safeReset(inp)
    }
  ...
  def charSet(cType:String, deFcs:String):String = {
    ...
    if (isEmpty(cs)) MimeUtility.javaCharset(deFcs) else cs
  }
  ...
  def smimeDigSig(key:PrivateKey, certs:Array[JCert], algo:SigningAlgo, mp:Multipart) = {
    ...
    var mm= newMimeMsg(); mm.setContent(mp)
...
*/
   ...
   * @throws CertificateEncodingException
   */
  def verifySmimeDigSig( mp:Multipart, certs:Array[JCert], cte:String="")
    :(Any,Array[Byte]) = {
    //tstArgIsType("multipart", mp, classOf[MimeMultipart])
    val mmp= mp.asInstanceOf[MimeMultipart]
    val sc= if (isEmpty(cte)) new SMIMESigned( mmp) else new SMIMESigned(mmp, cte)
  ...
  def compressContent(contentType:String, msg:XData):MimeBodyPart = {
    ...
    val ds= msg.fileRef() match {
      ...
      case _ => new  SmDataSource(msg.bytes(), contentType)
    }
   ...
   * @throws GeneralSecurityException
   */
  def compressContent(cType:String, cte:String,
      contentLoc:String, cid:String, msg:XData) = {
    val gen= new SMIMECompressedGenerator()
    var bp= new MimeBodyPart()
    var cID=cid
    val ds= msg.fileRef() match {
      case Some(f) =>    new    SmDataSource(f, cType )
      case _ => new SmDataSource(msg.bytes(), cType )
    }
  ...
  def pkcsDigSig(key:PrivateKey, certs:Array[JCert], algo:SigningAlgo, data:XData ) = {
    ...
    val p= Crypto.provider()
    val lst=LinkedList[JCert](); certs.foreach{ (c) => lst :+ c }
    ...
    val cms = data.fileRef() match {
      ...
      case _ =>  new CMSProcessableByteArray( data.bytes())
    }
  ...
  def certDesc( cert:JCert):(X500Principal,X500Principal,JDate,JDate) = {
    ...
    cert match {
      case x509:XCert =>
        val issuer= x509.getIssuerX500Principal()
        val subj= x509.getSubjectX500Principal()
        val vs = x509.getNotBefore()
        val ve = x509.getNotAfter()
        (subj, issuer, vs, ve)
      case _ => null
    }
  }

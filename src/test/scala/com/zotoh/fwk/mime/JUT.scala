...
class FwkMimeJUT  extends FunSuite with BeforeAndAfterEach with BeforeAndAfterAll  with Consts {
  ...
  test("testUrlEnc") {
    expect("abc")(urlEncode("abc"))
    assert("ab c" != urlEncode("ab c"))
  }

  test("testUrlDec") {
    expect("abc")(urlDecode("abc"))
    expect("ab c")(urlDecode( urlEncode("ab c")))
  }

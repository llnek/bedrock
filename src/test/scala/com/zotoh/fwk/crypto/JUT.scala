...
import com.zotoh.fwk.crypto.CryptoUte._
import com.zotoh.fwk.crypto.Crypto._

import java.io.{File,InputStream,ByteArrayOutputStream=>BAOS}
...
import com.zotoh.fwk.io.{XData,XStream}
import com.zotoh.fwk.mime.MimeUte
import com.zotoh.fwk.util.{FileUte,CoreImplicits,Consts}
...
class FwkCryptoJUT  extends FunSuite with BeforeAndAfterEach with BeforeAndAfterAll with CoreImplicits  {
  ...
  override def beforeAll(configMap: Map[String, Any]) {
    val cs=PKCSStore().init(new File( rc2Url("com/zotoh/fwk/crypto/test1.p12").getFile()), PWD1)
    val pk= cs.keyEntity( cs.keyAliases()(0), PWD1)
    assert(pk != None)
    _PKE=pk.get

    BaseOfuscator.setKey(
      CoreUte.asBytes("use-this-as-the-key-for-testing").slice(0, 24)
    )
  }
  ...
  test("testCaesar") {
    ...
    (1 to 18).foreach { (i) =>
      ...
      expect( base)( cc.decode( cc.encode(base) ) )
    }
    (1 to 18).foreach { (i) =>
  ...
  test("testDigestAlgo") {
    ...
    try {
      ...
      assert(false,"Unexpected new digest for (xxx) was OK!" )
    } catch {
      case e => assert(true)
    }
  ...
  test("testCertBytes") {
    val ks= KeyStore.getInstance("PKCS12")
    using(rc2Stream("com/zotoh/fwk/crypto/zotoh.p12")) { (inp) =>
      ks.load(inp, PWD1.toCharArray())
    }
    assert( ks.aliases().hasMoreElements())
  ...
  test("testCreateStores") {
    var c:CryptoStore= PKCSStore()
    c.init("xxx")
    expect(c.certAliases().length)(0)
    expect(c.keyAliases().length)(0)
    c= JKSStore()
    c.init("xxx")
    expect(c.certAliases().length)(0)
    expect(c.keyAliases().length)(0)
  ...
  test("testAddPKey") {
    val crs = Array( (PKCSStore(),"zotoh.p12"), ( JKSStore(), "zotoh.jks") )
    crs.foreach { (cs) =>
      cs._1.init("xxx")
      using(rc2Stream("com/zotoh/fwk/crypto/" + cs._2)) { (inp) =>
  ...
  test("testRemoveXXX") {
    ...
    crs.foreach { (cs) =>
      ...
      expect(cs._1.keyAliases().length)(0)
    }
  }

  test("testAddP7b") {
    val crs = Array( PKCSStore() )
    crs.foreach { (cs) =>
      cs.init("xxx")
      using(rc2Stream("com/zotoh/fwk/crypto/test2.p7b")) { (inp) =>
  ...
  test("testAddCert") {
    ...
    crs.foreach { (cs) =>
      cs.init("xxx")
      using(rc2Stream("com/zotoh/fwk/crypto/zotoh.cer")) { (inp) =>
  ...
  test("testGetRootCAs") {
    ...
    crs.foreach { (cs) =>
      ...
      expect(cs._1.intermediateCAs().length)(0)
    }
  }
  ...
  test("testKeyFac") {
    val crs = Array( PKCSStore(), JKSStore() )
    crs.foreach { (cs) =>
      cs.init("xxx")
  ...
  test("testBitsToCert") {
    using(rc2Stream("com/zotoh/fwk/crypto/zotoh.cer")) { (inp) =>
      assert(CryptoUte.bitsToCert(bytes(inp)) != None)
    }
  }

  test("testBitsToKey") {
    using(rc2Stream("com/zotoh/fwk/crypto/zotoh.p12")) { (inp) =>
      assert(CryptoUte.bitsToKey(bytes(inp), PWD1) != None)
    }
  }
  ...
  test("testGetCertDesc") {
    ...
    using(rc2Stream("com/zotoh/fwk/crypto/zotoh.cer")) { (inp) =>
      val bits= bytes(inp)
      var tc= CryptoUte.bitsToCert(bits)
      assert(tc != None)
      val c= tc.get.getTrustedCertificate()
      assert(c != null)
      val props= CryptoUte.certDesc(bits)
      assert(props._1 != null)
      val props2= CryptoUte.certDesc(c)
      assert(props == props2)
    }
  ...
  test("testNewRandom") {
    assert(CryptoUte.newRandom() != null)
  }

  test("testFingerPrints") {

    val s1 = CryptoUte.fingerPrintSHA1(CoreUte.asBytes("hello world"))
    assert(s1 != null && s1.length() > 0)
    val s2 = CryptoUte.fingerPrintMD5(CoreUte.asBytes("hello world"))
  ...
  test("testTstCertValid") {
    ...
    using(rc2Stream("com/zotoh/fwk/crypto/zotoh.cer")) { (inp) =>
      val ok=CryptoUte.tstCertValid(bytes(inp))
      assert(ok)
    }
  }
  ...
  test("testTstKeyValid") {
    ...
    // so should be true, unless you are stilling using this lib 100 years from now.
    using(rc2Stream("com/zotoh/fwk/crypto/zotoh.p12")) { (inp) =>
      val ok=CryptoUte.tstPKeyValid(bytes(inp), PWD1)
      assert(ok)
    }
  }

  test("testSessions") {
    val s0= CryptoUte.newSession("popeye", PWD1)
    val s1= CryptoUte.newSession()
    assert(s0 != null)
    assert(s1 != null)
  }
  ...
  test("testDSigMimeMsg") {
    var m= newMimeMsg(rc2Stream("com/zotoh/fwk/crypto/mime.txt"))
    var c:Any= m.getContent() match {
      case x:Multipart => x
      case _ =>
        assert(false,"Expecting object to be Multipart"); null
    }
    var mp=smimeDigSig(_PKE.getPrivateKey(), _PKE.getCertificateChain(),
        SigningAlgo.SHA512, c.asInstanceOf[Multipart])
    assert(MimeUte.isSigned(mp.getContentType()))
...
//    System.out.println(new String(os.asBytes()))
    ...
    m=newMimeMsg(rc2Stream("com/zotoh/fwk/crypto/mime.txt"))
  ...
  test("testEncryptMimeMsg") {
    ...
    var s=new SmDataSource( CoreUte.asBytes("hello world"), "text/plain")
    ...
    assert(MimeUte.isEncrypted( msg.getContentType() ))

    var dd= smimeDecryptAsStream(Array[PrivateKey]( _PKE.getPrivateKey()), msg)

    assert(dd != null)
    assert(asString(dd.bytes()).has("hello world"))

    s=new SmDataSource(CoreUte.asBytes("hello world"), "text/plain")
    bp = new MimeBodyPart()
    bp.setDataHandler(new DataHandler( s))
    ...
    bp.setDataHandler(new DataHandler( s))
    mp.addBodyPart(bp)
    s=new SmDataSource(CoreUte.asBytes("hello hello hello"), "text/plain")
    bp = new MimeBodyPart()
    bp.setDataHandler(new DataHandler( s))
    mp.addBodyPart(bp)
    bp=smimeEncrypt( _PKE.getCertificate(), EncryptionAlgo.AES256_CBC, mp)
    assert(MimeUte.isEncrypted(bp.getContentType()))

    dd=smimeDecrypt( _PKE.getPrivateKey(), bp)
    assert(dd != null)
    assert(asString(dd.bytes()).has("hello hello hello") )
...
//    System.out.println(new String(os.asBytes()))
  ...
  test("testCmpzion") {
    ...
    assert(MimeUte.isCompressed(bp.getContentType()))
...
//    System.out.println(new String(os.asBytes()))
    ...
    assert(asString(  dd.bytes()  ).has("zzz") )

  }

  test("testOfuscator") {
    val bo = JavaOfuscator()
    val data= Array( "holy batman", "", null )

    data.foreach { (c) =>
      var e= bo.obfuscate(c)
      if (c==null) {
        assert(e==null)
      } else if (e==null) {
        assert(c==null)

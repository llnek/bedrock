...
class SplitJUT () extends TestBase {

  test("test Split") {
    val ps=create_props("com.zotoh.bedrock.wflow.TestSplit")
    ps.put("_", this)
    _eng.start(ps)
...
class TestSplit(j:Job) extends Workflow(j) {
  ...
  def onStart() = {
    new Split().addSplit(s1).addSplit(s2).chain(new Delay(3000L)).chain(e)
  }
